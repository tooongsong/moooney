...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-paper border-t-2 border-ink px-6 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] flex justify-between items-center z-40 max-w-md mx-auto">
      <Link
        href="/"
        className={cn(
          'flex flex-col items-center gap-1 w-14 transition-colors',
      ...
      <Link
        ...
        className={cn(
          'flex flex-col items-center gap-1 w-14 transition-colors',
          pathname === '/accounts' ? 'text-ink' : 'text-ink-faint hover:text-ink-soft'
        )}
      >
        <Wallet className="h-5 w-5" strokeWidth={pathname === '/accounts' ? 2.5 : 2} />
        <span className="text-[10px] font-semibold uppercase tracking-wide">Accounts</span>
      </Link>
